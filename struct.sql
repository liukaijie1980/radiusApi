...
BEGIN
  #本存储过程目的是删除所有和ui元素失去关联的记录，时间可能很长，建议手动执行。
  DELETE
    FROM realm
...
#上面的方法因为要求严格的执行时机，被替换成下面的方法
CALL ManagePartitionsByDay('radpostauth',7);
CALL ManagePartitionsByMonth('offline_user',12);


#删除因为树节点的删除操作失去外键关联的记录(event_of_minute中删除一部分，下面的记录多所以移到这里)
DELETE FROM account_info WHERE realm NOT IN (SELECT realm FROM realm );

#标记那些在线但已经账号停止的用户，以便coa进程将通知bras将其踢下线
UPDATE   online_user  set kickme=1  WHERE  CONCAT(username,realm)  NOT IN   (  SELECT username FROM   view_radcheck);
