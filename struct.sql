...
(
PARTITION p20240320 VALUES LESS THAN (739331),
...
BEGIN
  -- 初始化变量
  DECLARE done int DEFAULT 0;  -- 循环结束的标志
  DECLARE ip varchar(255);  -- 存储从游标中提取的IP地址或范围
  DECLARE net varchar(255);  -- 存储IP网络地址
  DECLARE mask int;  -- 存储子网掩码
  DECLARE mask_offset int;  -- 用于计算网络地址的位偏移量
  DECLARE value_reversal tinyint(1);  -- 存储每个IP条目的reversal值
  DECLARE best_reversal tinyint(1) DEFAULT NULL;  -- 存储最匹配项的reversal值
  DECLARE max_mask int DEFAULT 0;  -- 存储最大匹配的子网掩码值

  -- 定义返回nas表中的nasname和reversal的游标
...
read_loop:
  LOOP
    FETCH cur INTO ip, value_reversal;  -- 从游标中提取数据到变量
    IF done THEN
      LEAVE read_loop;  -- 如果没有更多的行，退出循环
    END IF;

    -- 如果IP是'*'，则代表适用于所有地址，且没有比它优先级更低的（因为mask为0）
    IF ip = '*'
      AND max_mask <= 0 THEN
      SET max_mask = 0;
    ...
    ELSEIF INSTR(ip, '/') > 0 THEN
      SET net = SUBSTRING_INDEX (ip, '/', 1);  -- 分离出网络部分
      SET mask = CAST(SUBSTRING_INDEX (ip, '/', -1) AS UNSIGNED);  -- 分离出掩码
      SET mask_offset = 32 - mask;  -- 计算位偏移量
      -- 判断nasip是否在这个网络范围内
      IF (INET_ATON(nasip) >> mask_offset << mask_offset) = (INET_ATON(net) >> mask_offset << mask_offset)
        AND mask > max_mask THEN
        SET max_mask = mask;  -- 更新最大匹配的子网掩码
        SET best_reversal = value_reversal;  -- 更新对应的reversal值
      END IF;
    END IF;
  END LOOP;
  CLOSE cur;  -- 关闭游标

  RETURN best_reversal;  -- 返回找到的最合适的reversal值
